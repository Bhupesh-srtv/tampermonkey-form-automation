// ==UserScript==
// @name         HS-118 Daily Autofill
// @namespace    http://tampermonkey.net/
// @version      2.1
// @description  Autofills HS-118 and auto prints
...
(function() {
    ...
    function fillForm() {
        checkIt('tr:nth-child(2) [name="fav_input"]');
        setValue('textarea', 'COUNTERTOP INSTALL ONLY');
        setValue('[name="SPSign"]', 'Vendor name');
        setValue('[name="firmcertno"]', 'Firm Cert. no');
        setValue('[name="usercertno"]', 'Individual cert. No');
    ...
    window.addEventListener('load', function() {
        setTimeout(() => {
            if (document.body.innerText.includes('LSWP Exception Form')) {
                fillForm();
            }
        }, 3000);
    });
